        ...
        for i in self.partition_col:
            for d in self.date_col:
                ...
                if self.agg_col is None:
                    ...
                    daily_sum = (
                        X.groupby([i, d]).agg(value=(i, self.agg_value)).reset_index()
                    )
                    ...
                    daily_sum = (
                        X.groupby([i, d])
                        .agg(value=(self.agg_col, self.agg_value))
                        .reset_index()
                    )

                avg_daily_vol = (
                    daily_sum.groupby([i])
                    .agg(avg_value=("value", self.agg_type))
                    .to_dict()["avg_value"]
                )
...
class AggAmtperTxn(BaseEstimator, TransformerMixin):
    def __init__(self, partition_col, agg_col: str, date_col: List[str], agg_type: str):
    ...
    def fit(self, X, y=None):
        ...
        for i in self.partition_col:
            for d in self.date_col:
                ...
                daily_sum = (
                    X.groupby([i, d])
                    .agg(value=(self.agg_col, "sum"), count=(i, "count"))
                    .reset_index()
                )

                daily_sum["amt_per_txn"] = daily_sum.apply(
                    lambda x: x["value"] / x["count"], axis=1
                )

                avg_daily_sum_vol = (
                    daily_sum.groupby([i])
                    .agg(avg_value=("amt_per_txn", self.agg_type))
                    .to_dict()["avg_value"]
                )
...
class PurchaseType(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        for c in self.col:
            ...
            X[c + "_purchase_type"] = X[c].map(
                lambda x: "net"
                if re.search("net", str(x))
                else "pos"
                if re.search("pos", str(x))
                else "other"
            )
